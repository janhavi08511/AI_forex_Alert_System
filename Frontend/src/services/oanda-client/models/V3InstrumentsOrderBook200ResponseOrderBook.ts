...
import type { BookBucketsInner } from './BookBucketsInner';
import {
    BookBucketsInnerFromJSON,
    BookBucketsInnerFromJSONTyped,
    BookBucketsInnerToJSON,
    BookBucketsInnerToJSONTyped,
} from './BookBucketsInner';

/**
 * 
 * @export
 * @interface V3InstrumentsOrderBook200ResponseOrderBook
 */
export interface V3InstrumentsOrderBook200ResponseOrderBook {
    /**
     * The book's instrument. A string containing the base currency and quote currency delimited by a "_".
     * @type {string}
     * @memberof V3InstrumentsOrderBook200ResponseOrderBook
     */
    instrument: string;
    /**
     * The price (midpoint) for the book's instrument at the time of the book snapshot. A decimal number encodes as a string. The amount of precision provided depends on the Instrument.
     * @type {string}
     * @memberof V3InstrumentsOrderBook200ResponseOrderBook
     */
    price: string;
    /**
     * The price width for each bucket. Each bucket covers the price range from the bucket's price to the bucket's price + bucketWidth. A decimal number encodes as a string. The amount of precision provided depends on the Instrument.
     * @type {string}
     * @memberof V3InstrumentsOrderBook200ResponseOrderBook
     */
    bucketWidth: string;
    ...
    time: string;
    /**
     * The partitioned book, divided into buckets using a default bucket width. These buckets are only provided for price ranges which actually contain order or position data.
     * @type {Array<BookBucketsInner>}
     * @memberof V3InstrumentsOrderBook200ResponseOrderBook
     */
    buckets: Array<BookBucketsInner>;
}

/**
 * Check if a given object implements the V3InstrumentsOrderBook200ResponseOrderBook interface.
 */
export function instanceOfV3InstrumentsOrderBook200ResponseOrderBook(value: object): value is V3InstrumentsOrderBook200ResponseOrderBook {
    if (!('instrument' in value) || value['instrument'] === undefined) return false;
    if (!('price' in value) || value['price'] === undefined) return false;
    if (!('bucketWidth' in value) || value['bucketWidth'] === undefined) return false;
    if (!('time' in value) || value['time'] === undefined) return false;
    if (!('buckets' in value) || value['buckets'] === undefined) return false;
    return true;
}

export function V3InstrumentsOrderBook200ResponseOrderBookFromJSON(json: any): V3InstrumentsOrderBook200ResponseOrderBook {
    return V3InstrumentsOrderBook200ResponseOrderBookFromJSONTyped(json, false);
}

export function V3InstrumentsOrderBook200ResponseOrderBookFromJSONTyped(json: any, ignoreDiscriminator: boolean): V3InstrumentsOrderBook200ResponseOrderBook {
    if (json == null) {
        return json;
    }
    return {
        ...
        'time': json['time'],
        'buckets': ((json['buckets'] as Array<any>).map(BookBucketsInnerFromJSON)),
    };
}

export function V3InstrumentsOrderBook200ResponseOrderBookToJSON(json: any): V3InstrumentsOrderBook200ResponseOrderBook {
    return V3InstrumentsOrderBook200ResponseOrderBookToJSONTyped(json, false);
}

export function V3InstrumentsOrderBook200ResponseOrderBookToJSONTyped(value?: V3InstrumentsOrderBook200ResponseOrderBook | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
    ...
    return {
        ...
        'price': value['price'],
        'bucketWidth': value['bucketWidth'],
        'time': value['time'],
        'buckets': ((value['buckets'] as Array<any>).map(BookBucketsInnerToJSON)),
    };
}
